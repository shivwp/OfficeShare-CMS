...
<div class="card">
    ...
    <div class="card-body"id="add_space">
        <form action="{{ route("dashboard.blog.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                <label for="name">Is It Feature Post?</label><br>
                <input type="checkbox" name="feature" value="1" {{isset($edblog->feature) && $edblog->feature==1?"checked":''}}>
            </div>
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                <label for="name">Is It Sticky Post?</label><br>
                <input type="checkbox" name="sticky" value="1" {{isset($edblog->sticky) && $edblog->sticky==1?"checked":''}}>
            </div>
            <div>
